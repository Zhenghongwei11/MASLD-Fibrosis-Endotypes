...
def stage_results(run_id: str) -> None:
    ...
    for gse in ["GSE163211", "GSE48452", "GSE49541", "GSE89632", "GSE130970", "GSE162694", "GSE135251"]:
        try:
    ...
    subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "pipeline" / "meta_analysis_sensitivity.py"),
            "--effect-tsv",
            str(REPO_ROOT / "results" / "endotypes" / "endotype_fibrosis_effects_multicohort.tsv"),
            "--out-tsv",
            str(REPO_ROOT / "results" / "effect_sizes" / "meta_analysis_endotype_f3plus_sensitivity.tsv"),
        ],
        check=True,
    )
