...
def fit_nmf_scores(expr_log: pd.DataFrame, k: int) -> pd.DataFrame:
...
def main() -> int:
    ...
    rows = []
    for k in ks:
        Wz = fit_nmf_scores(expr_log, k=k)
        for col in Wz.columns:
            or_, lo, hi, p = assoc_or_per_1sd(y, Wz[col])
            rows.append(
                {
                    "dataset_id": GSE,
                    "outcome": "discovery_label_MASH_F1_F4_vs_others",
                    "k": k,
                    ...
                    "pvalue": p,
                    "n": int(Wz.shape[0]),
                    "label_positive": int(y.loc[Wz.index].sum()),
                }
            )
