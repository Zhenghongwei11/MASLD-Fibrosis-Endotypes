...
def plot_endotype_scores_by_stage(scores_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    fig.suptitle("Endotype scores by NAFLD stage (GSE163211)", y=1.02, fontsize=11)
    fig.tight_layout()
...
def plot_endotype_effects(effect_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    ax.set_title("Endotype score association with fibrotic steatohepatitis (F1–F4 label; GSE163211)")
    ax.invert_yaxis()
    fig.tight_layout()
...
def plot_forest_meta(effect_tsv: Path, meta_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    for idx, (ax, feat) in enumerate(zip(axes, features, strict=True)):
        ...
        if not m.empty:
            ...
            ax.set_title(
                f"{_pretty_endotype_label(feat)}: pooled OR={pooled:.2f} (I²={float(m['I2_percent']):.0f}%)",
                fontsize=10,
            )
        else:
    ...
    axes[-1].set_xlabel("Odds ratio per 1-SD endotype score (log scale)")
    fig.suptitle("Multi-cohort association with advanced fibrosis (≥F3 vs ≤F2)", y=1.01, fontsize=10.5)
    fig.tight_layout()
...
def plot_loco_performance(loco_predictions_or_eval_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    fig.suptitle("External validation summary (leave-one-cohort-out holdout)", y=1.02, fontsize=10.5)
    fig.tight_layout()
...
def plot_prediction_srt_style(loco_predictions_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    axA.text(
        0.07,
        0.92,
        f"Pooled LOCO: n={int(len(y_all))}, events={int(y_all.sum())}",
...
def plot_robustness_sensitivity(nmf_sensitivity_tsv: Path, out_dir: Path) -> list[Path]:
    ...
    # Summarize by (k, component): distribution across seeds
    ...
    fig.suptitle("Robustness: discovery endotype associations across NMF settings", y=1.02, fontsize=10.5)
    fig.tight_layout()
