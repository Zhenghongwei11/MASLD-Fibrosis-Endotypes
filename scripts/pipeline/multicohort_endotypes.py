...
def _transformation_record(gse: str, platform_id: str) -> dict[str, str]:
    if gse == "GSE130970":
        return {
            "dataset_id": gse,
            "platform_id": platform_id,
            "input_scale": "TPM",
            "expression_transformation": "log2(TPM+1)",
            "scoring_scale": "within-cohort gene-wise z-score followed by signature mean",
        }
    if gse in {"GSE162694", "GSE135251"}:
        return {
            "dataset_id": gse,
            "platform_id": platform_id,
            "input_scale": "raw counts",
            "expression_transformation": "library-size CPM then log2(CPM+1)",
            "scoring_scale": "within-cohort gene-wise z-score followed by signature mean",
        }
    if gse == DISCOVERY_GSE:
        return {
            "dataset_id": gse,
            "platform_id": platform_id,
            "input_scale": "processed targeted NanoString series-matrix values",
            "expression_transformation": "log1p(non-negative expression) for NMF discovery; processed values for transfer coverage",
            "scoring_scale": "within-cohort gene-wise z-score followed by signature mean",
        }
    return {
        "dataset_id": gse,
        "platform_id": platform_id,
        "input_scale": "processed GEO series-matrix values",
        "expression_transformation": "as supplied by GEO series matrix after probe-to-gene aggregation",
        "scoring_scale": "within-cohort gene-wise z-score followed by signature mean",
    }
...
def reml_hartung_knapp(theta: np.ndarray, se: np.ndarray) -> dict[str, float]:
    ...
    pred_sd = float(math.sqrt(tau2 + se_mu * se_mu))
    pi_lo = mu - tcrit * pred_sd
    pi_hi = mu + tcrit * pred_sd
    return {
        "method": "REML_HKSJ",
        "k": float(theta.size),
        "mu_log_or": mu,
...
def run(
    ...
    *,
    ...
    for gse in cohorts:
        ...
        transform_rows.append(_transformation_record(gse, platform_id))
        ...
        y_nonmissing = y2.dropna()
        if y_nonmissing.nunique(dropna=True) < 2:
            continue
    ...
    if transform_rows:
        pd.DataFrame(transform_rows).drop_duplicates().to_csv(
            figures_dir / "expression_transformation_summary.tsv", sep="\t", index=False
        )
